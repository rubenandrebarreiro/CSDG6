...
//                                  Defreezing the money of the last top bid
                                ...
                                return new JSONObject().put("error", "Auction " + auctionId + " already contains a Bid with the id " + bidId);
                        } else {
    ...
    public static JSONObject validateCode(Class<?> cl) {
        AuctionCreatorSmartContractByteCodeVerifier v = new AuctionCreatorSmartContractByteCodeVerifier(cl);
        try {
            v.verifyConstructors();
            v.verifyFields();
            v.verifyInterfaces();
        } catch (Exception e) {
